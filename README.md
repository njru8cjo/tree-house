...
This is an example of an out-of-tree [MLIR](https://mlir.llvm.org/) dialect along with a tree `opt`-like tool to operate on that dialect.
...
cmake -G Ninja `$LLVM_SRC_DIR/llvm` \
    ...
    -DLLVM_EXTERNAL_PROJECTS=tree-dialect -DLLVM_EXTERNAL_TREE_DIALECT_SOURCE_DIR=../
cmake --build . --target check-tree
```
